...
<?php echo $HtmlHelper->input('_cf_table_name', array('label' => 'Table Name', 'value' => '#__chronoforms_data_'.$row->name, 'class' => 'medium_input', 'smalldesc' => 'Enter the table name here, no spaces or any special characters, underscores allowed.')); ?>
...
<thead>
	...
	<th width="10%" align="left" class='title' style="text-align: left;">Primary key</th>
...
<?php if(!empty($defaults)): ?>
	...
	<?php foreach($defaults as $default => $default_data): ?>
		<tr>
			...
			<td width="10%" align="left" class='title'><input type="checkbox" onclick="select_one(<?php echo $i; ?>)" id="field_key_<?php echo $i; ?>" value="1" name="field_key[<?php echo $i; ?>]"<?php if($default_data['key'] == 'PRI')echo ' checked'; ?> /></td>
...
<?php for($x = 0; $x <= 30; $x++): ?>
	<tr>
		...
		<td width="10%" align="left" class='title'><input type="checkbox" onclick="select_one(<?php echo $i; ?>)" id="field_key_<?php echo $i; ?>" value="1" name="field_key[<?php echo $i; ?>]" /></td>
